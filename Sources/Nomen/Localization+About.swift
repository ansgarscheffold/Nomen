...
extension L10n {
    ...
    var aboutCredits: String {
        s(
            en: """
            PDFs with embedded text (digital invoices etc.) are read directly via PDFKit (first 3 pages for naming). \
            Scanned PDFs fall back to Vision OCR on the first 3 pages. Other file types: PDFKit or plain text. \
            ...
            """,
            de: """
            PDFs mit eingebettetem Text (digitale Rechnungen usw.) werden direkt via PDFKit gelesen (für die Benennung die ersten 3 Seiten). \
            Gescannte PDFs nutzen Vision-OCR auf den ersten 3 Seiten als Fallback. Andere Dateitypen: PDFKit oder Klartext. \
